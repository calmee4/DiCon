...
from utils.util import multi_label_metric, ddi_rate_score, get_n_params, create_log_id, logging_config, get_model_path, get_grouped_metrics,ddi_rate_score_positive
...
parser.add_argument('--resume_path',type = str ,default="./saved/mimic-iv/MICRON/Epoch_50_JA_0.4474_DDI_0.06664.model") #Epoch_50_JA_0.4474_DDI_0.06664.model
args = parser.parse_args()
def eval(model, data_eval, voc_size, epoch,
         ddi_adj_path, ddi_adj_path_positive,
         val=0, threshold1=0.8, threshold2=0.2, rec_results_path=None):
    model.eval()
    ...
    for step, input in tqdm(enumerate(data_eval), ncols=60, total=len(data_eval), desc="Evaluation"):
        ...
        for adm_idx, adm in enumerate(input):
            ...
            if adm_idx == 0:
                ...
                y_old = np.zeros(voc_size[2], dtype=np.float32)
                y_old[adm[2]] = 1
                ...
                y_pred_bin = (y_pred_tmp >= 0.5).astype(np.float32)
                y_pred.append(y_pred_bin)   # 这里是新数组，不会被后续修改影响

                y_pred_label_tmp = np.where(y_pred_bin == 1)[0]
                ...
                y_pred.append(y_old.copy())  # ★重要：用 copy，避免下次迭代原地修改影响之前的记录
        ...
        if args.test:
            (ja_visit[len(input)-1] if len(input) < 5 else ja_visit[4]).append(adm_ja)
            rec_results.append([all_diseases, all_procedures, all_medications,
    ...
    logging.info(
        'Jaccard: {:.4}, DDI Rate: {:.4}, DDI Rate Positive: {:.4}, PRAUC:{:.4}  '
        'AVG_F1: {:.4}, AVG_PRC: {:.4}, AVG_RECALL: {:.4f}, AVG_MED: {:.4}'.format(
            np.mean(ja), ddi_rate, ddi_rate_positive, np.mean(prauc),
            np.mean(avg_f1), np.mean(avg_p), np.mean(avg_r), med_cnt / visit_cnt
        )
    )

    if val == 0:
        # ★现在返回 6 个指标，包含 Positive DDI（第三个）
...
def bootstrap_eval(
    ...
    输出：ja / ddi_rate / ddi_rate_positive / prauc / avg_f1 / med 的 mean ± std
    """
    rng = np.random.default_rng(seed)
    N = len(data_test_raw); k = int(round(N * ratio))
    rows = []

    for _ in range(rounds):
        idx = rng.choice(N, size=k, replace=True).tolist()
        subset = [data_test_raw[i] for i in idx]
        ddi_rate, ddi_pos, ja, prauc, avg_f1, avg_med = eval(
            model, subset, voc_size, epoch=0,
            ddi_adj_path=ddi_adj_path,
            ddi_adj_path_positive=ddi_adj_path_positive,
            val=0, threshold1=threshold1, threshold2=threshold2,
            rec_results_path=None
        )
        rows.append([ja, ddi_rate, ddi_pos, prauc, avg_f1, avg_med])

    arr = np.array(rows, dtype=np.float64)
    mean, std = arr.mean(axis=0), arr.std(axis=0)
    names = ['ja', 'ddi_rate', 'ddi_rate_positive', 'prauc', 'avg_f1', 'med']

    print('--------------------Bootstrap (MICRON)--------------------')
...
def main():
    ...
    if args.test:
        ...
        label_list, prob_list = eval(model, data_eval, voc_size, epoch=0, ddi_adj_path=ddi_adj_path, ddi_adj_path_positive=ddi_adj_path_positive,val=1, rec_results_path = rec_results_path)
        ...
        eval(model, data_test, voc_size, epoch=0, ddi_adj_path=ddi_adj_path, ddi_adj_path_positive = ddi_adj_path_positive,val=1, threshold1=threshold1, threshold2=threshold2, rec_results_path = rec_results_path)
        logging.info('test time: {}'.format(time.time() - tic))
        bootstrap_eval(
            model, data_test, voc_size,
            ddi_adj_path=ddi_adj_path,
            ddi_adj_path_positive=ddi_adj_path_positive,
            rounds=10, ratio=0.8, seed=0,
    ...
    for epoch in range(EPOCH):
        ...
        ddi_rate, ddi_rate_pos, ja, prauc, avg_f1, avg_med = eval(model, data_eval, voc_size, epoch, ddi_adj_path,ddi_adj_path_positive)

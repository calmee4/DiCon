...
def get_args():
    ...
    parser.add_argument('--resume_path' ,type=str, default="./saved/mimic-iv/GAMENet/Epoch_50_JA_0.4403_DDI_0.07349.model")
    ...
    parser.add_argument('--early_stop', type=int, default=20, help='early stop after this many epochs without improvement')
...
def evaluator(args, model, data_val, voc_size, epoch, 
    ...
    logging.info(f'''Epoch {epoch:03d}, Jaccard: {np.mean(ja):.4}, DDI Rate: {ddi_rate:.4}, Positive DDI Rate: {ddi_rate_positive:.4},PRAUC: {np.mean(prauc):.4}, AVG_F1: {np.mean(avg_f1):.4},  AVG_PRC: {np.mean(avg_p):.4f}, AVG_RECALL: {np.mean(avg_r):.4f}, AVG_MED: {med_cnt / visit_cnt:.4}''')
...
def test_bootstrap(
    ...
    *, ddi_adj_path, ddi_adj_path_positive, rounds=10, ratio=0.8, seed=0
    ...
    for _ in range(rounds):
        ...
        # 用 evaluator 做一次评测；注意 mode='Eval' 且不落盘
        ddi_rate, ddi_rate_positive, ja, prauc, avg_f1, avg_med = evaluator(
            ...
            rec_results_path=None, mode='Eval'
        )
...
def main(args):
    ...
    for epoch in range(EPOCH):
        ...
        ddi_rate, ddi_rate_positive ,ja, prauc, f1,  avg_med = evaluator(
            args, model, data_val, voc_size, epoch, ddi_adj_path,ddi_adj_path_positive)

...
from utils.util import multi_label_metric, ddi_rate_score,ddi_rate_score_positive,get_n_params, create_log_id, logging_config, get_grouped_metrics, \
...
ddi_adj_positive_path = f'../../data/output/mimic-iii'+ '/ddi_B_final.pkl'
...
def eval_one_epoch(model, data_eval, voc_size):
    ...
    # 新增：存储不同 visit 次数的 Jaccard
    visit_ja_sum = {}
    visit_ja_cnt = {}
    max_visits_to_report = 5
    ...
    for step, input_seq in enumerate(data_eval):
        ...
        for adm_idx, adm in enumerate(input_seq):
            ...
            y_pred_tmp[y_pred_tmp >= args.threshold] = 1
            y_pred_tmp[y_pred_tmp < args.threshold] = 0
        ...
        # 累积 visit-level Jaccard
        num_visits = len(input_seq)
        if num_visits <= max_visits_to_report:
            visit_ja_sum[num_visits] = visit_ja_sum.get(num_visits, 0) + adm_ja
            visit_ja_cnt[num_visits] = visit_ja_cnt.get(num_visits, 0) + 1
    ...
    # === 输出 1..5 次访问的 Jaccard ===
    for k in range(1, max_visits_to_report + 1):
        if visit_ja_cnt.get(k, 0) > 0:
            mean_ja = visit_ja_sum[k] / visit_ja_cnt[k]
            print(f"Visit {k} 平均 Jaccard: {mean_ja:.4f} (n={visit_ja_cnt[k]})")
        else:
            print(f"Visit {k} 平均 Jaccard: N/A (该序号访问不存在)")
...
def Test(model, device, data_test, voc_size,data):
    ...
    for _ in range(10):
        ...
        ddi_rate, ddi_rate_positive, ja, prauc, avg_p, avg_r, avg_f1, avg_med, avg_fp, avg_fn \
...
def get_args():
    ...
    parser.add_argument('--early_stop', type=int, default=50, help='early stop after this many epochs without improvement')
    ...
    parser.add_argument('--resume_path', default="./saved/mimic-iv/RAREMed/Epoch_9_JA_0.4506_DDI_0.06969.model", type=str,
...
def eval_one_epoch_by_visit(model, data_eval, voc_size, threshold=0.5, max_visits=5):
    """
    按 visit 统计 Jaccard / PRAUC / F1
    """
    model.eval()
    metrics_by_visit = [dict(ja=[], prauc=[], f1=[]) for _ in range(max_visits)]
    for step, patient in enumerate(data_eval):
        for adm_idx, adm in enumerate(patient):
            # forward 用到当前 patient 的历史
            output, _, _ = model(patient[:adm_idx+1])
            output = torch.sigmoid(output).detach().cpu().numpy()[0]
            ...
            # prediction
            y_pred_tmp = (output >= threshold).astype(int)
            ...
            # 统计到对应 visit 桶
            idx = adm_idx if adm_idx < max_visits else max_visits-1
            metrics_by_visit[idx]["ja"].append(adm_ja)
            metrics_by_visit[idx]["prauc"].append(adm_prauc)
            metrics_by_visit[idx]["f1"].append(adm_avg_f1)

    # 打印结果
    for i, m in enumerate(metrics_by_visit):
        if len(m["ja"]) > 0:
            print(f"Visit {i+1}: "
                  f"JA mean={np.mean(m['ja']):.4f}, std={np.std(m['ja']):.4f} | "
                  f"PRAUC mean={np.mean(m['prauc']):.4f}, std={np.std(m['prauc']):.4f} | "
                  f"F1 mean={np.mean(m['f1']):.4f}, std={np.std(m['f1']):.4f} "
                  f"(n={len(m['ja'])})")
        else:
            print(f"Visit {i+1}: no samples")

    return metrics_by_visit


# evaluate
@torch.no_grad()
def evaluator(args, model, data_val, voc_size, epoch, 
              ddi_adj_path,ddi_adj_positive_path, device, rec_results_path = ''):
    model.eval()
    ...
    loss_val_bce = loss_val_milti = loss_val_ddi=loss_val_ddi_positive = loss_val_sum = 0                # 统计验证集总loss
    ...
    for patient in tqdm(data_val, ncols=60, total=len(data_val), desc=f"Evaluation"):
        ...
        results, loss_ddi,loss_ddi_positive = model(patient)
        ...
        loss_val_ddi_positive += loss_ddi_positive.item() / len_val
    ...
    ddi_rate_positive = ddi_rate_score_positive(smm_record,path=ddi_adj_positive_path)
    print("*"*5+"ddi-rate-positive"+"*"*5)
    print(ddi_rate_positive)
    get_grouped_metrics(ja, visit_weights)
    logging.info(f'''Epoch {epoch:03d}, Jaccard: {np.mean(ja):.4}, DDI Rate: {ddi_rate:.4} , DDI Rate Positive : {ddi_rate_positive:.4} , PRAUC: {np.mean(prauc):.4}, AVG_F1: {np.mean(avg_f1):.4}, AVG_PRC: {np.mean(avg_p):.4f}, AVG_RECALL: {np.mean(avg_r):.4f}, AVG_MED: {med_cnt / visit_cnt:.4}''')
    
    loss_val_sum = (1 - args.weight_multi) * loss_val_bce + args.weight_multi * loss_val_milti + args.weight_ddi * loss_val_ddi - args.weight_ddi*loss_val_ddi_positive

    return ddi_rate, ddi_rate_positive , np.mean(ja), np.mean(prauc), np.mean(avg_f1), med_cnt / visit_cnt,\
          loss_val_bce, loss_val_milti, loss_val_ddi, loss_val_ddi_positive,loss_val_sum
...
def evaluator_mask(model, data_val, voc_size, epoch, device, mode='pretrain'):
    ...
    for batch in tqdm(data_val, ncols=60, desc=mode, total=len_val):
        ...
        dis_ja, dis_prauc, dis_avg_p, dis_avg_r, dis_avg_f1,fp,fn  = multi_label_metric(
            np.array(dis_gt), np.array(dis_pred), np.array(dis_pred_prob))
        pro_ja, pro_prauc, pro_avg_p, pro_avg_r, pro_avg_f1,fp,fn = multi_label_metric(
...
def main(args):
    ...
    # 嵌入的size-> diag pro med 病情 手术 药物
    voc_size = (len(diag_voc.idx2word), len(pro_voc.idx2word), len(med_voc.idx2word))
    # model initialization
    # print(voc_size)
    model = RAREMed(args, voc_size, ddi_adj,ddi_adj_positive)
    logging.info(model)
#第一个需要改的地方
    if args.test:
        ...
        with open(args.resume_path, 'rb') as Fin:
            model.load_state_dict(torch.load(Fin, map_location=device))
        eval_one_epoch_by_visit(model,data_test,voc_size,0.5,5)
        eval_one_epoch(model,data_test,voc_size)
        Test(model, device, data_test, voc_size, data)

        logging.info("load model from %s", args.resume_path)
        rec_results_path = save_dir + '/' + 'rec_results'

        # evaluator(args, model, data_test, voc_size, 0, ddi_adj_path,ddi_adj_positive_path, device, rec_results_path)
        return
    else:
    ...
    for epoch in range(EPOCH):
        ...
        # finetune
        model.train()
        loss_train_bce, loss_train_multi, loss_train_ddi, loss_train_ddi_positive , loss_train_all = 0, 0, 0, 0,0
        ...
        for step, patient in tqdm(enumerate(data_train), ncols=60, desc="finetune", total=len(data_train)):  # every patient
            result, loss_ddi,loss_ddi_positive = model(patient)
            loss_bce, loss_multi = loss_func(voc_size, patient, result, device)
            loss_all = (1-args.weight_ddi)*((1 - args.weight_multi) * loss_bce + args.weight_multi * loss_multi )+ args.weight_ddi * loss_ddi
            ...
            loss_train_ddi_positive += loss_ddi_positive.item() / len(data_train)
            loss_train_all += loss_all.item()/len(data_train)

        # evaluation
        ddi_rate, ddi_rate_positive, ja, prauc, avg_f1, avg_med,\
        loss_val_bce, loss_val_multi, loss_val_ddi,loss_val_ddi_positive, loss_val_all = evaluator(
            args, model, data_val, voc_size, epoch, ddi_adj_path,ddi_adj_positive_path, device)
        ...
        logging.info(f'''loss_train_all:{loss_train_all:.4f}, loss_train_bce: {loss_train_bce:.4f}, loss_train_multi:{loss_train_multi:.4f}, loss_train_ddi:{loss_train_ddi:.4f}, 
                                loss_val_all:  {loss_val_all:.4f}, loss_val_bce:  {loss_val_bce:.4f}, loss_val_multi:  {loss_val_multi:.4f}, loss_val_ddi:  {loss_val_ddi:.4f} , loss_val_ddi_positive:  {loss_val_ddi_positive:.4f} ''')
...
def tensorboard_write(writer, ja, prauc, ddi_rate,ddi_rate_positive, avg_med, epoch,
    ...
    writer.add_scalar('Metrics/DDI_Positive', ddi_rate_positive, epoch)

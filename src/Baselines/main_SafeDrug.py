...
from utils.util import multi_label_metric, ddi_rate_score, \
    get_n_params, buildMPNN, create_log_id, logging_config, get_grouped_metrics, get_model_path,ddi_rate_score_positive
...
def get_args():
    ...
    parser.add_argument("--resume_path",type=str,default="./saved/mimic-iii/SafeDrug/Epoch_25_JA_0.5255_DDI_0.07508.model")
    args = parser.parse_args()
    return args

# evaluate
@torch.no_grad()
def evaluator(model, data_val, voc_size, epoch, ddi_adj_path,ddi_adj_path_positive, rec_results_path=None):
    model.eval()
    ...
    ddi_rate_positive= ddi_rate_score_positive(smm_record, path=ddi_adj_path_positive)
    get_grouped_metrics(ja, visit_weights)

    logging.info(f'''Epoch {epoch:03d}, Jaccard: {np.mean(ja):.4}, DDI Rate: {ddi_rate:.4}, PRAUC: {np.mean(prauc):.4},DDI RATE POSITIVE :{ddi_rate_positive:.4},AVG_F1: {np.mean(avg_f1):.4}, AVG_PRC: {np.mean(avg_p):.4f}, AVG_RECALL: {np.mean(avg_r):.4f}, AVG_MED: {med_cnt / visit_cnt:.4}''')
...
def test_bootstrap(
    ...
    统计: ja / ddi_rate / prauc / avg_f1 / med 的 mean ± std，并写日志。
    """
    ...
    for _ in range(rounds):
        ...
        ddi_rate, ddi_rate_positive, ja, prauc, avg_f1, avg_med = evaluator(
            model, subset, voc_size, epoch=0,
            ddi_adj_path=ddi_adj_path,
            ddi_adj_path_positive=ddi_adj_path_positive,
            rec_results_path=None
        )
        rows.append([ja, ddi_rate, ddi_rate_positive,avg_f1, prauc, avg_med])
...
def main(args):
    ...
    if args.test:
        ...
        evaluator(model, data_test, voc_size, 0, ddi_adj_path, ddi_adj_path_positive,rec_results_path)
        return 
    else:
    ...
    for epoch in range(EPOCH):
        ...
        # evaluation
        ddi_rate, ddi_rate_positive, ja, prauc, avg_f1, avg_med = evaluator(
            model, data_val, voc_size, epoch, ddi_adj_path, ddi_adj_path_positive)
    ...
    test_bootstrap(args,model,device,data_test,voc_size,ddi_adj_path,ddi_adj_path_positive,rounds=10, ratio=0.8, seed=0)

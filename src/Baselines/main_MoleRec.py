...
from utils.util import buildPrjSmiles, create_log_id, logging_config, get_model_path, \
    multi_label_metric, ddi_rate_score, get_grouped_metrics, get_n_params,ddi_rate_score_positive,llprint
...
def parse_args():
    ...
    parser.add_argument('-l', '--log_dir_prefix', type=str, default="", help='log dir prefix like "log0", for model test')
    parser.add_argument('--cuda', type=int, default=5, help='which cuda')
    parser.add_argument('--resume_path', default="./saved/mimic-iii/MoleRec/Epoch_11_JA_0.5274_DDI_0.0755.model", type=str,
...
def eval_one_epoch(model, dataset, data_eval, voc_size, drug_data, mode = 'Val', rec_results_path=None):
    ...
    ddi_rate = ddi_rate_score(smm_record, path=f"../../data/output/{args.dataset}/ddi_A_final.pkl")
    ...
    return ddi_rate,ddi_rate_positive,np.mean(ja),np.mean(prauc),np.mean(avg_p),np.mean(avg_r),np.mean(avg_f1),med_cnt/visit_cnt


def Test(model, dataset, model_path, device, data_test, voc_size, drug_data, rec_results_path, n_boot=10, sample_frac=0.8, rng_seed=0):
    if model_path != None:
    ...
    for b in range(n_boot):
        ...
        sample = [data_test[i] for i in idx]

        ddi_rate, ddi_rate_positive, ja, prauc, avg_p, avg_r, avg_f1, avg_med = \
            eval_one_epoch(model, dataset, sample, voc_size, drug_data, "Test", rec_results_path)

        stats.append([ddi_rate, ddi_rate_positive, ja, prauc, avg_p, avg_r, avg_f1, avg_med])

    stats = np.asarray(stats)
    mean = stats.mean(axis=0)
    std  = stats.std(axis=0)

    keys = ["ddi_rate", "ddi_rate_positive", "ja", "prauc", "avg_p", "avg_r", "avg_f1", "avg_med"]
    res = {
        "mean": {k: float(m) for k, m in zip(keys, mean)},
        "std":  {k: float(s) for k, s in zip(keys, std)},
        "raw":  stats,
    }
    print(res)
    logging.info(res)
...
def Train(
    ...
    torch.save(best_model_state, open(os.path.join(save_dir, \
                                                   'Epoch_{}_JA_{:.4f}_DDI_{:.4f}.model'.format(best_epoch,
                                                                                                float(best_ja),
                                                                                                float(best_ddi_rate))),
                                      'wb'))
...
if __name__ == '__main__':
    ...
    ddi_adj_positive_path = f'../../data/output/{dataset}/ddi_B_final.pkl'
    ...
    if args.embedding:
        ...
        #molecule_para and substruct_para
    model = MoleRecModel(
    ...
    if args.test:
        ...
        Test(model, dataset, model_path, device, data_test, voc_size, drug_data, model_path)
    else:

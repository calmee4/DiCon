...
class RAREMed(PatientEncoder):
    def __init__(self, args, voc_size, ddi_adj,ddi_adj_positive):
        super(RAREMed, self).__init__(args, voc_size)
        self.tensor_ddi_adj = torch.FloatTensor(ddi_adj).to(self.device)
        self.tensor_ddi_adj_positive = torch.FloatTensor(ddi_adj_positive).to(self.device)
    ...
    def forward_finetune(self, input):
        ...
        # result = result - result.mean(dim=1) + output_mean_0
        batch_neg_positive = 0.0001 * neg_pred_prob.mul(self.tensor_ddi_adj_positive).sum()

        return result, batch_neg, batch_neg_positive

    def forward(self, input, mode='fine-tune'):
        assert mode in ['fine-tune', 'pretrain_mask', 'pretrain_nsp']
        if mode == 'fine-tune':
            result, batch_neg,batch_neg_positive = self.forward_finetune(input)
            return result, batch_neg,batch_neg_positive

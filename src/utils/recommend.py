...
from utils.util import llprint, sequence_output_process, ddi_rate_score, output_flatten,\
      multi_label_metric, pop_metric, get_grouped_metrics,ddi_rate_score_positive
...
def eval(args, epoch, model, eval_dataloader, voc_size, ddi_adj_path, rec_results_path = None):
    ...
    logging.info(f'''Epoch {epoch:03d}, Jaccard: {np.mean(ja):.4}, DDI Rate: {ddi_rate:.4},DDI Rate Positive: {ddi_rate_positive:.4}, PRAUC: {np.mean(prauc):.4},  AVG_PRC: {np.mean(avg_p):.4f}, AVG_RECALL: {np.mean(avg_r):.4f}, AVG_F1: {np.mean(avg_f1):.4}, AVG_MED: {med_cnt / visit_cnt:.4}''')
